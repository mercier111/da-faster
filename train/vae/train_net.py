...
from model.detect_vae.resnet import resnet
from model.detect_vae.vgg16 import vgg16
...
from show_pic import tensor2numpy
...
def parse_args():
  ...
  """
    ...
    parser.add_argument(
        "--lamda", dest="lamda", help="DA loss param", default=0.01, type=float
    )

    parser.add_argument(
        "--lamda2", dest="lamda2", help="DA loss param", default=100, type=float
    )
    ...
    parser.add_argument(
        "--use_tensor_board", dest="use_tensor_board", help="useing tensor borad", action="store_true"
    )
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            loss = (
                ...
                + RCNN_loss_cls.mean()
                + RCNN_loss_bbox.mean()
                + rand_rpn_loss_cls.mean()
                + rand_rpn_loss_bbox.mean()
                + rand_RCNN_loss_cls.mean()
                + rand_RCNN_loss_bbox.mean()
                + tgt_rand_rpn_loss_cls.mean()
                + tgt_rand_rpn_loss_bbox.mean()
                + tgt_rand_RCNN_loss_cls.mean()
                + tgt_rand_RCNN_loss_bbox.mean()
                + tgt_rand2_rpn_loss_cls.mean()
                + tgt_rand2_rpn_loss_bbox.mean()
                + tgt_rand2_RCNN_loss_cls.mean()
                + tgt_rand2_RCNN_loss_bbox.mean()
                + args.lamda * (source_cyc_loss.mean()
                             + target_cyc_loss.mean())
                + args.lamda2 * sorce_map_loss.mean()
            ...
            if (step % 200 == 0) and args.use_tensor_board :

                source = np.concatenate((tensor2numpy(im_data), 
                                         tensor2numpy(source_im_data), 
                                         tensor2numpy(source_rand_data))
                                         , 1)
                source_gt = np.concatenate((tensor2numpy(im_data, gt_boxes), 
                                            tensor2numpy(source_im_data, gt_boxes), 
                                            tensor2numpy(source_rand_data, rand_gt_boxes))
                                            , 1)
                target = np.concatenate((tensor2numpy(tgt_im_data), 
                                         tensor2numpy(target_im_data), 
                                         tensor2numpy(target_rand_data), 
                                         tensor2numpy(target_rand2_data)), 
                                         1)
                target_gt = np.concatenate((tensor2numpy(tgt_im_data), 
                                            tensor2numpy(target_im_data), 
                                            tensor2numpy(target_rand_data, tgt_rand_gt_boxes), 
                                            tensor2numpy(target_rand2_data, tgt_rand2_gt_boxes)), 
                                            1)
                _source = np.concatenate((source, source_gt), 0)
                _target = np.concatenate((target, target_gt), 0)

                writer.add_images('source', _source, real_step, dataformats='HWC')
                writer.add_images('target', _target, real_step, dataformats='HWC')
                real_step += 1
            ...
            if step % args.disp_interval == 0:
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\trand_rpn_cls: %.4f, rand_rpn_box: %.4f, rand_rcnn_cls: %.4f, rand_rcnn_box %.4f,\n\t\t\ttgt_rand_rpn_cls: %.4f, tgt_rand_rpn_box: %.4f, tgt_rand_rcnn_cls: %.4f, tgt_rand_rcnn_box %.4f,\n\t\t\ttgt_rand2_rpn_cls: %.4f, tgt_rand2_rpn_box: %.4f, tgt_rand2_rcnn_cls: %.4f, tgt_rand2_rcnn_box %.4f,\n\t\t\tsource_cyc_loss %.4f, target_cyc_loss %.4f,source_map_loss: %.4f"
                    % (
                        loss_rpn_cls,
                        loss_rpn_box,
                        loss_rcnn_cls,
                        ...
                        tgt_rand2_RCNN_loss_cls,
                        tgt_rand2_RCNN_loss_bbox,
                        args.lamda * source_cyc_loss, 
                        args.lamda * target_cyc_loss,
                        args.lamda2 * sorce_map_loss,
                    )
                )
