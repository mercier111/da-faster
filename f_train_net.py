...
from model.f_faster_rcnn.resnet import resnet
from model.f_faster_rcnn.vgg16 import vgg16
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        for step in range(iters_per_epoch):
            ...
            loss = (
                rpn_loss_cls.mean()
                + rpn_loss_bbox.mean()
                + two_rpn_loss_cls.mean()
                + two_rpn_loss_bbox.mean()
                + RCNN_loss_cls.mean()
                + RCNN_loss_bbox.mean()
                + args.lamda
                * (
                   two_RCNN_DA_cls.mean()
                   + two_RCNN_DA_bbox.mean()
                   + tgt_RCNN_DA_cls.mean()
                   + tgt_RCNN_DA_bbox.mean()
                )
            )
            loss_temp += loss.item()
            ...
            if step % args.disp_interval == 0:
                ...
                two_loss_rpn_cls = two_rpn_loss_cls.item()
                two_loss_rpn_bbox = two_rpn_loss_bbox.item()
                loss_rcnn_cls = RCNN_loss_cls.item()
                loss_rcnn_bbox = RCNN_loss_bbox.item()
                two_RCNN_DA_cls = two_RCNN_DA_cls.item()
                two_RCNN_DA_bbox = two_RCNN_DA_bbox.item()
                tgt_RCNN_DA_cls = tgt_RCNN_DA_cls.item()
                tgt_RCNN_DA_bbox = tgt_RCNN_DA_bbox.item()
                ...
                print(
                    "\t\t\trpn_cls: %.4f, rpn_box: %.4f, two_rpn_cls: %.4f, two_rpn_box %.4f\n\t\t\tRCNN_loss_cls %.4f, RCNN_loss_bbox %.4f\n\t\t\ttwo_RCNN_DA_cls %.4f, two_RCNN_DA_bbox %.4f\n\t\t\ttgt_RCNN_DA_cls %.4f, tgt_RCNN_DA_bbox %.4f"
                    % (
                        loss_rpn_cls,
                        loss_rpn_bbox,
                        two_loss_rpn_cls,
