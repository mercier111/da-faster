...
from model.cycle_faster_rcnn.faster_rcnn import _fasterRCNN
...
model_urls = {
    ...
    "resnet101": "datasets/pretrained_model/resnet101.pth",
...
class resnet(_fasterRCNN):
    def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False):
        self.model_path = "datasets/pretrained_model/resnet101.pth"
    ...
    def _init_modules(self):
        ...
        self.tgt_RCNN_cls_score = nn.Linear(2048, self.n_classes)

        if self.class_agnostic:
            self.RCNN_bbox_pred = nn.Linear(2048, 4)
            self.tgt_RCNN_bbox_pred = nn.Linear(2048, 4)
        else:
            self.RCNN_bbox_pred = nn.Linear(2048, 4 * self.n_classes)
            self.tgt_RCNN_bbox_pred = nn.Linear(2048, 4 * self.n_classes)
